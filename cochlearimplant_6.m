...
function band = bp(sample, Fs, f_low, f_high)
    bp = designfilt('bandpassfir','FilterOrder',50, ...
         'CutoffFrequency1',2*(f_low-10)/Fs,'CutoffFrequency2',2*(f_high+10)/Fs, ...
         'SampleRate',Fs*1);
...
function final_sound = coswave(filename, envelopes, y, Fs, n)
    ...
    %plot(final_sound);
    %sound(final_sound);
    audiowrite("C:\Users\Sarah Dykstra\Documents\2B\252\Project\SoundFiles\6\final-"+filename, final_sound, Fs);
end