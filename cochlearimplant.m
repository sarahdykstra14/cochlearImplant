...
function band = bp(sample, Fs, f_low, f_high)
    bp = designfilt('bandpassiir', 'FilterOrder', 30, 'StopbandFrequency1', ...
                f_low-10, 'StopbandFrequency2', f_high+10, ...
...
function [y,envelopes] = filters(sample, Fs, n_channels)
    ...
    y=logspace(log10(110),log10(7990),n_channels+1);
    for n = 1:n_channels
...
function final_sound = coswave(filename, envelopes, y, Fs, n)
    ...
    %plot(final_sound);
    %sound(final_sound);
    audiowrite("C:\Users\Sarah Dykstra\Documents\2B\252\Project\SoundFiles\4\final-"+filename, final_sound, Fs);
end