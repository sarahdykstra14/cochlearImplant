...
function band = bp(sample, Fs, f_low, f_high)
    bp = designfilt('bandpassiir', 'FilterOrder', 30, 'StopbandFrequency1', ...
                f_low-30, 'StopbandFrequency2', f_high+30, ...
...
function [y,envelopes] = filters(sample, Fs, n_channels)
    ...
    y=logspace(log10(120),log10(7969),n_channels+1);
    for n = 1:n_channels
...
function final_sound = coswave(filename, envelopes, y, Fs, n)
    ...
    %plot(final_sound);
    %sound(final_sound);
    audiowrite("C:\Users\Sarah Dykstra\Documents\2B\252\Project\SoundFiles\2\final-"+filename, final_sound, Fs);
end